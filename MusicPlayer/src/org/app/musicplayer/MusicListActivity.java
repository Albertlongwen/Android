...
protected void onResume() {
	...
	private void ShowMp3List() {
		...
		if (cursor!= null && cursor.getCount() == 0) {
			final XfDialog xfdialog = new XfDialog.Builder(MusicListActivity.this).setTitle("提示:").setMessage("你的手机未找到音乐,请添加音乐").setPositiveButton("确定", null).create();
			xfdialog.show();
			return;
 ...
 */
	...
	private void SongItemDialog() {
		...
		menulist.setAdapter(new ArrayAdapter<String>(MusicListActivity.this,R.layout.dialog_menu_item, R.id.text1, menustring));
		menulist.setLayoutParams(new LayoutParams(Contsant.getScreen(MusicListActivity.this)[0] / 2,LayoutParams.WRAP_CONTENT));

		final XfDialog xfdialog = new XfDialog.Builder(MusicListActivity.this).setTitle(R.string.opertion_file).setView(menulist).create();
		xfdialog.show();
	...
	private void SetRing() {
		...
		new XfDialog.Builder(MusicListActivity.this).setTitle("设置铃声").setView(rg_ring).setPositiveButton("确定", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						dialog.cancel();
						dialog.dismiss();
  ...
  public void setAlarm(int position) 
	...
	private void ShowMusicInfo(int location) {
		...
		new XfDialog.Builder(MusicListActivity.this).setTitle(R.string.music_info).setNeutralButton(R.string.confrim, null).setView(view).create().show();
	}
	/**
	 * 休眠方法
	 */
   private void Sleep(){
	   ...
	   edtext.setText("5");//设置初始值
	    ...
	    new XfDialog.Builder(MusicListActivity.this).setTitle("请输入时间").setView(edtext).setPositiveButton("确定", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if (edtext.length() <= 2 && edtext.length() != 0) {
					if (".".equals(edtext.getText().toString())) {
						toast = Contsant.showMessage(toast,MusicListActivity.this, "输入错误，你至少输入两位数字.");
					} else {
					    ...
					    timer.start();//倒计时开始
						toast = Contsant.showMessage(toast,MusicListActivity.this, "休眠模式启动!于" + String.valueOf(time)+ "分钟后关闭程序!");
					...
					Toast.makeText(MusicListActivity.this, "请输入几分钟",Toast.LENGTH_SHORT).show();
				}
